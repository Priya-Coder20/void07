...
export default function StaffSchedule() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error(error);
            setMessage('Error uploading schedule');
        }
    };

    return (
        <div className="max-w-2xl mx-auto bg-white p-8 rounded-lg shadow">
            <h2 className="text-2xl font-bold mb-6 text-gray-800">Upload Schedule</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    <label className="block text-gray-700 mb-1">File URL / Content</label>
                    <input
                        type="text"
                        value={formData.fileUrl}
                        onChange={(e) => setFormData({ ...formData, fileUrl: e.target.value })}
                        className="w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500 text-black"
                        placeholder="Link to schedule or text content"
                    />
                </div>
                <button
                    type="submit"
